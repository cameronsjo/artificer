// Pure open-state decisions for collapsible `.sidenav__section`s. Kept free of
// React/DOM so the rules are unit-testable without mounting a component or
// faking matchMedia. Lifted upstream from spec-compare (proven in production
// with its test suite) as part of the chrome-components mint.
/**
 ...
 */
export function defaultSectionOpen(viewport, isActive) {
 ...
 */
export function onViewportChange(state, viewport, isActive) {
    if (state.touched)
        return state;
 ...
 */
export function onNavigationActivates(state) {
...
export function onUserToggle(nextOpen) {
