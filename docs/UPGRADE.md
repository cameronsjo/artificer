...
| **0.23.0 → 0.24.0** | The **chrome-components minor**. Import the compiled React adapter — `import { Appbar, NavDrawer, SideNav, SideNavFooter, ThemeToggle, AppShell } from '@cameronsjo/artificer/react'` (ESM + `d.ts`; `react >=18` peer) — and delete your hand-rolled chrome; the vanilla modules keep owning behavior. Ambient types for the vanilla modules now ship (`types/*.d.ts`, wired to the `./theme.js`-style exports) — delete hand-copied `focus.d.ts`/`icons.d.ts`/`tabs.d.ts`/`whimsy.d.ts`. `.app-shell` gains `--shell-gap`; `.sidenav--sticky` mints — replace any local `.app-shell` grid override / sticky wrapper aside with the knobs. Move the dependency from devDependencies to dependencies (the adapter is runtime source). | **`useTheme` (copy-only React hook) is REMOVED** — it was 2-state and duplicated the module's persistence. If you copied `framework-adapters/react-components.tsx`, migrate to the canonical `<ThemeToggle />` (module-owned state) and `useThemeMode()` for reactive reads. Sections mode (`<SideNav sections>`) needs the active section's group flagged `active` on its items or a deep link lands with the active row hidden. |
...
| **0.22.0 → 0.22.1** | Mint `.btn--link` — a `<button>` that renders as a link, for controls that **act** rather than navigate (an SPA switching views, a disclosure toggle, "show more"). Use it instead of dressing an `<a>` in a click handler, which makes the accessibility tree announce a link that doesn't link. It carries the 44×44 touch floor itself. If your colophon spine's links slot holds `<button>`s, add `class="btn btn--link"`. | **None forced — additive.** On 0.22.0 a bare `<button>` in a spine renders as raw browser chrome; this is the supported fix. |
