...
- **[verified 2026-08-20, from source]** Per-element override is **all-or-nothing**: setting any one attribute on an `<option>` element disables inheritance of the rest — set `FOREGROUND` and you also drop the inherited `BACKGROUND`/`FONT_TYPE` to defaults, so plan each token's full attribute set. The rule is verbatim in the original SDK doc ([color-scheme-management](https://plugins.jetbrains.com/docs/intellij/color-scheme-management.html)) and is consistent with every real-world `.icls` examined, *but* no genuinely **independent** (different-author) source restates it — all apparent second sources are verbatim mirrors/translations of the same JetBrains doc, and third-party articles that might confirm it returned 403. Settled from the platform source rather than a second article: `EditorColorsSchemeImpl.getAttributes()` returns a directly-defined `TextAttributes` object whole when present and only falls through to the fallback key / parent when none is defined — there is no per-field merge. The Artificer emitter therefore writes each entry's full intended set.
