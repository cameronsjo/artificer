...
export function ThemeToggle({
  ...
  className,
  ...rest
}: React.ButtonHTMLAttributes<HTMLButtonElement> & { inline?: boolean }) {
  return (
    <button
      type="button"
      data-theme-toggle
      ...
      {...rest}
      className={cx('theme-toggle', inline && 'theme-toggle--inline', className)}
    />
  );
}
 ...
 */
export function useThemeMode(): { theme: 'dark' | 'light'; toggle: () => void } {
  ...
  const toggle = React.useCallback(() => {
    (window as unknown as { ArtificerTheme?: { toggle?: () => void } }).ArtificerTheme?.toggle?.();
  }, []);
  return { theme, toggle };
}
...
export function AppShell({
  ...
  if (rail) vars['--shell-rail'] = rail;
  if (gap) vars['--shell-gap'] = gap;
  return (
    <div
      {...rest}
...
export function AppShellContent({
  ...
  return <As {...rest} className={cx('app-shell__content', className)} />;
}
...
export function Appbar({
  ...
  menu?: { controls: string; open: boolean; onClick: () => void; label?: string };
} ) {
  return (
    <header
      {...rest}
      className={cx('appbar', contained && 'appbar--contained', !sticky && 'appbar--static', className)}
    >
      {menu && (
        <button
          type="button"
          className="btn btn--ghost btn--icon btn--icon-prominent appbar__menu-btn"
          aria-controls={menu.controls}
          aria-expanded={menu.open}
          aria-label={menu.label ?? (menu.open ? 'Close navigation' : 'Open navigation')}
          onClick={menu.onClick}
        >
          <i data-icon="menu" aria-hidden="true" />
        </button>
      )}
      <a className="appbar__brand" href={safeHref(brandHref) ?? '/'}>
        <span className={cx('wordmark', brandWhimsy && 'whimsy')}>{brand}</span>
      </a>
      {search && <div className="appbar__search">{search}</div>}
...
export function NavDrawer({
  ...
  return (
    <div style={{ display: 'contents' }} {...(open ? { 'data-nav-open': '' } : {})}>
      <div className="nav-scrim" onClick={onClose} />
      <aside
        ref={ref}
        id={id}
        className={cx('nav-drawer', className)}
        aria-label={label}
        aria-hidden={!open}
      >
        {children}
...
function SideNavRow({ item }: { item: SideNavItem }) {
  const inner = (
    <>
      {item.icon && <i data-icon={item.icon} aria-hidden="true" />}
  ...
  return href ? (
    <a href={href} {...current} onClick={item.onSelect}>
      {inner}
    </a>
  ) : (
    <button type="button" {...current} onClick={item.onSelect}>
      {inner}
    </button>
  );
}
...
function groupIsActive(group: SideNavGroup): boolean {
...
export function SideNav({
  ...
  const [openMap, setOpenMap] = React.useState<Record<string, SectionOpenState>>(() =>
    Object.fromEntries(
      groups.map((g) => [g.key, { open: defaultSectionOpen('desktop', groupIsActive(g)), touched: false }]),
    ),
  );
  ...
  const prevActive = React.useRef<Record<string, boolean>>({});
  React.useEffect(() => {
    if (!sections) return;
    setOpenMap((m) => {
      let next = m;
  ...
  React.useEffect(() => {
    if (!sections || typeof window === 'undefined') return;
    const mq = window.matchMedia(TABLET_QUERY);
    const applyViewport = () => {
      const vp: Viewport = mq.matches ? 'mobile' : 'desktop';
      setOpenMap((m) =>
        Object.fromEntries(
          groups.map((g) => {
  ...
  return (
    <nav {...rest} className={cx('sidenav', sticky && 'sidenav--sticky', className)}>
      {groups.map((g) =>
        sections ? (
          <details
            key={g.key}
            className="sidenav__section"
            open={openMap[g.key]?.open ?? true}
            onToggle={(e: React.SyntheticEvent<HTMLDetailsElement>) => {
              const nextOpen = e.currentTarget.open;
              setOpenMap((m) => {
                const state = m[g.key];
                if (state && state.open === nextOpen) return m; // programmatic echo, not a user toggle
          ...
          >
            ...
            {g.items.map((item) => (
              <SideNavRow key={item.key} item={item} />
            ))}
          </details>
        ) : (
          <React.Fragment key={g.key}>
            <div className="sidenav__group">{g.label}</div>
            {g.items.map((item) => (
              <SideNavRow key={item.key} item={item} />
            ))}
          </React.Fragment>
...
export function SideNavFooter({
  ...
  return (
    <div {...rest} className={cx('sidenav__footer', className)}>
      <span>{label}</span>
